...
function add_lead_calllogs_tab($lead_id)
{
    ...
    $table_js = json_encode($table_html);
	...
	<script>
		$(document).on("shown.bs.modal", ".lead-modal", function () {
		  ...
		  $("body").on("click", ".view-group", function(){
			...
			$("#callLogsGroupModal .modal-body").html(html);
...
$(function(){
	...
	bc.onmessage = function(e){
		if (e.data === "close_call_modal") {
		...
		}else if(e.data === "open_call_modal"){
			...
			if(nextData){
				...
				$("#callDetailsModal").one("hidden.bs.modal", function(){
				  runQueue(nextData);
				  $("#callDetailsModal").one("shown.bs.modal", function(){
					skipBroadcast = false;
				  });
				});
			}
	...
	$(document).on("hide.bs.modal", "#callDetailsModal", function(){
    ...
    /*var $clientSel = $("#clientSelect");
    var $leadSel   = $("#prospectiveSelect");*/

    // 1. On load, disable the non-active select
    //$leadSel.prop("disabled", true);

    // 2. When tabs switch, disable the one you’re hiding, enable the one you’re showing
    /*$("a[data-toggle='."tab".']").on("shown.bs.tab", function(e){
        var target = $(e.target).attr("href"); // "#tab-clients" or "#tab-prospective"
        if (target === "#tab-clients") {
            $clientSel.prop("disabled", false);
            $leadSel.prop("disabled", true);
        } else {
            $leadSel.prop("disabled", false);
            $clientSel.prop("disabled", true);
        }
    });*/
        ...
        $.ajax({
            ...
            if (res.success) {
              ...
              if(queue.length > 0){
                ...
                bc.postMessage("open_call_modal");
              }
          } else {
